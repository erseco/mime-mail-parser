...
EOF;
        ...
        ->and($message->getParts()[0]->getContent())->toBe(
            <<<EOF
<html>
<head>
<title>This is an HTML email</title>
...
it(
    'can parse a multi-format mail message from file with attachment', function () {
        ...
        expect($parts)->toHaveCount(2)
            ...
            ->and($parts[1]->getContentType())->toBe('application/pdf; name=8790810DS5088N0001XJ.pdf')
            ->and($parts[1]->getHeaders())->toBe(
                [
                'Content-Type' => 'application/pdf; name=8790810DS5088N0001XJ.pdf',
                'Content-Disposition' => 'attachment; filename=8790810DS5088N0001XJ.pdf',
                'Content-Transfer-Encoding' => 'base64',
                // 'Content-Transfer-Encoding' => 'quoted-printable',
                ]
            )->and($message->getTextPart()?->getContent())->toBe(
...
it(
    'can parse a multi-format mail message from file with multiple attachments', function () {
        ...
        expect($message->getFrom())->toBe('John Doe <test@example.com>')
        ->and($message->getTo())->toBe('John Smith <sender@example.com>')
        ->and($message->getSubject())->toBe('Test Email - Formatting Sample')
        ->and($message->getDate()->format('Y-m-d H:i:s'))->toBe('2024-11-19 04:30:32')
        ->and($message->getBoundary())->toBe('=_cFbSYdhlnpfsJi5rQk-R8OA');
